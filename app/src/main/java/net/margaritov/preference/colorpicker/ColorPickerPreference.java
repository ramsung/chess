...
public class ColorPickerPreference
    ...
    private void setPreviewColor() {
        ...
        LinearLayout widgetFrameView = ((LinearLayout) mView.findViewById(
    ...
    public int getValue() {
        try {
            if (isPersistent()) {
                /*
                 * doesn't work in MD Chess, cause the values for the color
    ...
    public void onColorChanged(int color) {
        if (isPersistent()) {
            /* for MD Chess: the colors are stored as strings [2012-08-30; leo] */
